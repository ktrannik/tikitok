...
async def handle_message(update: Update, context):
    ...
    if not hashtags:
        await update.message.reply_text("❌ Напиши хештег с решёткой, например: #коты")
        return
...
if __name__ == '__main__':
    ...
    # Устанавливаем вебхук (используем путь, Railway сам подставит домен)
    ...
    if railway_domain:
        ...
        # Если нет переменной, используем относительный путь
        # Railway сам проксирует запросы к Flask
        webhook_url = f"https://{request.host}{webhook_path}" if request else None
        print("RAILWAY_PUBLIC_DOMAIN не задан")
    
    if webhook_url:
        async def setup():
            result = await telegram_app.bot.set_webhook(url=webhook_url)
            print(f"✅ Вебхук установлен: {webhook_url}" if result else "❌ Ошибка!")
        asyncio.run(setup())
    else:
        print("⚠️ Не могу установить вебхук, нет домена")
